...
class Conv2d_MLP_VariationalAutoencoder(nn.Module):
    def __init__(
        self,
        input_shape: Sequence[int],
        output_shape: Sequence[int],
        ...
        self.encoder = Conv2d_MLP_Block(
            input_shape=input_shape,
            output_shape=[latent_dim * 2],
            out_chans_per_layer=out_chans_per_layer, # For Conv2D
            out_chans_per_layer_MLP=out_chans_per_layer_MLP, # For Linear layers
            kernel_size_per_layer=kernel_size_per_layer,
            stride_per_layer=stride_per_layer,
            use_batch_norm=use_batch_norm,
        )
        ...
        out_chans_per_layer = out_chans_per_layer[::-1][1:] + [input_shape[0]]
        out_chans_per_layer_MLP = out_chans_per_layer_MLP[::-1]
        ...
        self.decoder = ConvTranspose2d_MLP_Block(
            input_shape=[latent_dim],
            output_shape=output_shape,
            linear_out_chan=linear_out_chan,
            out_chans_per_layer=out_chans_per_layer,
            out_chans_per_layer_MLP=out_chans_per_layer_MLP,
            kernel_size_per_layer=kernel_size_per_layer,
            stride_per_layer=stride_per_layer,
            use_batch_norm=use_batch_norm,
        )
